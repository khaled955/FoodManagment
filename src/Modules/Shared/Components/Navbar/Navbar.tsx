...
export default function Navbar({ onOpenMobileMenu }: Props) {
  // Auth Context
  const userContext = useContext(UserToken);
  if (!userContext) throw new Error("ProtectedRoute must be used within a UserAuthProvider");
  const { userAuth } = userContext;
  ...
  return (
    <nav className="px-0 px-sm-4 d-flex justify-content-between py-1 px-1 gap-3 position-fixed top-0 end-0 start-0 align-items-center my-nav">
      {/* MOBILE BURGER (xs–md). Previously had no onClick and was 'd-sm-none'. */}
      <button
        type="button"
        className="btn-show btn p-0 d-md-none"
        aria-label="Open menu"
        onClick={onOpenMobileMenu}
      >
        <i className="fa-solid fa-bars fs-4 text-black"></i>
      </button>
